...
def get_avg_precision_at_iou(gt_boxes, pred_boxes, iou_thr=0.5):
    ...
    for img_id in pred_boxes.keys():
        ...
        if 'class' in pred_boxes[img_id]: # TODO or re add the class field when splitting
            pred_boxes[img_id]['class'] = np.array(pred_boxes[img_id]['class'])[arg_sort].tolist()
    ...
    for ithr, model_score_thr in enumerate(sorted_model_scores[:-1]):
        ...
        for img_id in img_ids:
            ...
            if 'class' in pred_boxes[img_id]:  # TODO or re add the class field when splitting
                pred_boxes_pruned[img_id]['class'] = pred_boxes_pruned[img_id]['class'][start_idx:]
...
def split_boxes_by_class(gt_boxes, pred_boxes, classification: str):
    ...
    for gt_image in gt_boxes:
        ...
        for gt_object_box, gt_object_class in zip(gt_boxes[gt_image]['boxes'], gt_boxes[gt_image]['class']):
    ...
    for pred_image in pred_boxes:
        ...
        for pred_object_box, pred_object_score, pred_object_class in \
                zip(pred_boxes[pred_image]['boxes'], pred_boxes[pred_image]['scores'], pred_boxes[pred_image]['class']):
...
if __name__ == "__main__":
    ...
    # Run per class mAP calculation for one IoU threshold, 0.5
    iou_thr = 0.5
    ...
    # print('avg precs: ', avg_precs)
    # print('iou_thrs:  ', iou_thrs)
    # plt.title('Precision-Recall curve for Source')
    # plt.legend(loc='upper right', title='IOU Thr', frameon=True)
    # for xval in np.linspace(0.0, 1.0, 11):
    #     plt.vlines(xval, 0.0, 1.1, color='gray', alpha=0.3, linestyles='dashed')
    # end_time = time.time()
    ...
    # print('avg precs: ', avg_precs)
    # print('iou_thrs:  ', iou_thrs)
    # plt.title('Precision-Recall curve for Sidelobe')
    # plt.legend(loc='upper right', title='IOU Thr', frameon=True)
    # for xval in np.linspace(0.0, 1.0, 11):
    #     plt.vlines(xval, 0.0, 1.1, color='gray', alpha=0.3, linestyles='dashed')
    # end_time = time.time()
    ...
    # print('avg precs: ', avg_precs)
    # print('iou_thrs:  ', iou_thrs)
    # plt.title('Precision-Recall curve for Galaxy')
    # plt.legend(loc='upper right', title='IOU Thr', frameon=True)
    # for xval in np.linspace(0.0, 1.0, 11):
    #     plt.vlines(xval, 0.0, 1.1, color='gray', alpha=0.3, linestyles='dashed')
    # end_time = time.time()
